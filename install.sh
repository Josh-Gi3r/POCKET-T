#!/usr/bin/env bash
# pocket-t installer — single entry point for installing pocket-t.
#
...
# ─── 5. Install the `pocket` launcher ─────────────────────────────────────

# A tiny global wrapper so users don't have to memorise the full
# `pnpm --filter @pocket-t/daemon pt-registry …` invocation. Run `pocket`
# from any terminal — defaults to `serve --tunnel`. All other CLI
# subcommands (list, kill, replay, status, pending, approve, …) pass
# through unchanged: `pocket list`, `pocket kill <id>`, etc.
#
# The wrapper hard-codes this repo's location so it keeps working even
# from a terminal that has no notion of cwd. If the user moves the repo
# they just re-run `bash install.sh` from the new location.
bold "→ installing the 'pocket' launcher to /usr/local/bin/pocket…"

POCKET_WRAPPER="$(mktemp)"
cat > "$POCKET_WRAPPER" <<EOF
#!/usr/bin/env bash
# pocket — Pocket-T daemon launcher.
# Auto-generated by install.sh. Do not edit by hand; re-run install.sh.

set -e
POCKET_T_DIR="\${POCKET_T_DIR:-$REPO_ROOT}"
if [ ! -d "\$POCKET_T_DIR" ]; then
  echo "pocket: repo not found at \$POCKET_T_DIR" >&2
  echo "  Re-run 'bash install.sh' from the repo root, or set POCKET_T_DIR." >&2
  exit 1
fi
if [ \$# -eq 0 ]; then
  # Default: start the daemon and open a Cloudflare Quick Tunnel.
  exec pnpm --dir "\$POCKET_T_DIR" --filter @pocket-t/daemon pt-registry serve --tunnel
fi
# Otherwise pass everything through to the pt-registry CLI.
exec pnpm --dir "\$POCKET_T_DIR" --filter @pocket-t/daemon pt-registry "\$@"
EOF
chmod 755 "$POCKET_WRAPPER"
sudo install -m 755 "$POCKET_WRAPPER" /usr/local/bin/pocket
rm -f "$POCKET_WRAPPER"
green "✓ pocket installed: $(which pocket)"
...
echo
yellow "2) Start the daemon — open any terminal (not one going through pt) and run:"
echo
...
echo "       pocket serve --relay wss://your-hub"
echo "     See docs/self-hosting.md."
echo
echo "   ► Other useful subcommands:"
echo "       pocket list           — list active sessions"
echo "       pocket kill <id>      — kill a session"
echo "       pocket replay <id>    — replay a recorded session"
echo "       pocket pending        — list pending tool-call approvals"
echo

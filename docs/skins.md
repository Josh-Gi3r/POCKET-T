...
1. Open `packages/daemon/src/pt-registry/ui/index.html`.
2. Find the `:root` block at the top of the `<style>`. The built-in
   skins live just below it as `body[data-theme="…"]` blocks.
...
5. Mirror those changes in the hub's embedded page,
   `packages/relay/src/wsv3-hub.ts`, so remote browsers see the same
   skin when they connect through a self-hosted hub.
...
1. Fork the repo, add your skin to both `pt-registry/ui/index.html` and

...
zero-config, and token-gated (the printed URL carries the daemon's access
token), but ephemeral — the URL changes on every restart. This runbook wires
the durable version, adding a stable hostname and an identity check at the edge
so access doesn't rest on the URL alone:
...
- **tmux-resurrect / tmux-continuum** so your shells and agents come back
  after a reboot, not just after a terminal quit,
- **caffeinate** so the Mac keeps serving with the lid closed.

## The persistence model (what survives what)

Pocket-T already gives you three tiers of durability out of the box, and
this runbook adds the fourth:

| Event | Survives by default? | How |
|---|---|---|
| Quit Terminal.app / log out | ✅ | The shell runs in the private `pocket-t` tmux server, which outlives the `pt` client. |
| Daemon restart / upgrade | ✅ | The registry rehydrates from `state.json` + the live tmux server; browsers get the last screen + recent bubbles. |
| **Reboot** | ⚠️ **opt-in** | A process can't outlive the kernel. You need launchd to bring the daemon back **and** `tmux-resurrect`/`tmux-continuum` to restore the tmux sessions (steps 7–7b below). |

So "always-on" across a reboot is: daemon auto-started on login (step 7) +
tmux sessions restored on boot (step 7b) + the tunnel already up as a
LaunchDaemon (step 5).
...
## 7b. Restore shells across a reboot (tmux-resurrect)

Pocket-T runs every shell inside the private `pocket-t` tmux server, so a
terminal quit or a daemon restart never loses your work. A **reboot** is
different: the tmux server is a process, and no process outlives the
kernel. To bring your sessions back after a reboot, add
[`tmux-resurrect`](https://github.com/tmux-plugins/tmux-resurrect) (saves
and restores the session/window/pane layout, working directories, and
optionally the programs that were running) and
[`tmux-continuum`](https://github.com/tmux-plugins/tmux-continuum)
(auto-saves on an interval and auto-restores when the server starts).

Install via [tpm](https://github.com/tmux-plugins/tpm) and configure them
in `~/.tmux.conf` (tmux reads it regardless of the `-L` server label):
...
set -g @continuum-restore 'on'     # restore the last save when the server starts
set -g @continuum-save-interval '5' # auto-save every 5 minutes
set -g @resurrect-processes 'ssh vim nvim htop "claude" "codex"'  # re-run these on restore
run '~/.tmux/plugins/tpm/tpm'
EOF
```

Load the plugins into the pocket-t server once so they're active:

```bash
tmux -L pocket-t new-session -d -s bootstrap \; run '~/.tmux/plugins/tpm/tpm' \; kill-session -t bootstrap
```

Then after a reboot, the pocket-t tmux server comes back up (started on
demand by `pt`, or by continuum's own boot integration) and continuum
restores the last saved layout, at which point the daemon adopts the
restored tmux sessions and they reappear in the browser.

> **Honest caveat.** Resurrect restores the *layout*, working directories,
> and can *re-run* named programs — it does **not** snapshot a live
> process's memory. A long-running agent comes back as a freshly launched
> agent in the same directory, not paused mid-thought. For durability of
> the daemon's own catalog across a reboot, that's fine: it rehydrates
> from whatever tmux sessions exist plus `state.json`.
...
2. Reboot the Mac (don't log in). The tunnel LaunchDaemon should be up
   immediately; the daemon LaunchAgent starts at your login. With step 7b
   in place, continuum restores your tmux sessions and the daemon adopts
   them — the sessions you had open reappear in the browser.

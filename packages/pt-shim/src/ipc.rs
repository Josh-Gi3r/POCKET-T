...
impl DaemonConn {
    ...
    ///
    /// `tmux` reports how this session obtained its PTY: `true` when the
    /// shell runs inside a detached tmux session (survives shim/terminal
    /// exit), `false` for a direct forkpty child. The daemon uses it to
    /// pick the right liveness check on rehydrate.
    ...
    pub fn send_register(
        ...
        let json = format!(
            "{{\"sessionId\":\"{}\",\"cwd\":\"{}\",\"pid\":{},\"rows\":{},\"cols\":{},\"shell\":\"{}\",\"tmux\":{}}}",

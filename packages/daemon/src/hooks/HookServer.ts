...
export interface HookServerOpts {
   ...
   * across the user's entire machine would hang for `timeoutMs` waiting
   * on a UI that may never connect.
   */
   ...
   *  so the tool call is never left hanging. */
...
export class HookServer extends EventEmitter {
  ...
  private async handle(
    ...
    if (req.method === 'POST' && req.url === '/hook/preToolUse') {
      ...
      // Safety valve: if nobody is watching for approvals (no browser
      // tab open, no relay connection alive), respond immediately rather
      // than blocking the Claude tool call. The decision is
      // defaultOnNoListener, which the caller sets from exposure
      // (loopback → approve, tunnel/relay → deny); Claude's own
      // permission system still gates dangerous tools.
  ...
  // `MultiEdit`, `Read`, ...). Normalize case, gate write/edit tools
  // explicitly, and fail CLOSED: anything not provably read-only
  // requires approval.

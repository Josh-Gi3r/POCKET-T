...
//
// The query token `t` is the routing key. In a production relay this is
// a JWT verified against the accounts table; in this minimal hub it is
// any shared string the daemon and browser agree on (e.g. the user's
// daemonId), with the relay just matching strings.
//
...
<script>
  ...
  function renderAuth() {
    document.getElementById('root').innerHTML = \`
      <div class="auth">
        <h1>pocket-t hub</h1>
        <p>Paste your account token (in the MVP hub, this is any shared
        string the daemon dials in with — typically your daemonId).</p>
        <input id="token" placeholder="account token" autofocus>
  ...
  function boot(token) {
    ...
    function connect() {
      ...
      ws.addEventListener('open', () => {
        ...
        ws.send(encodeFrame(T.HELLO, '', new Uint8Array([3])));
        if (currentSession) {
